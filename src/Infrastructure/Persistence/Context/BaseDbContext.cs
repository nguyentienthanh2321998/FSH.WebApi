...
public abstract class BaseDbContext : IdentityDbContext<ApplicationUser, ApplicationRole, int, IdentityUserClaim<int>, IdentityUserRole<int>, IdentityUserLogin<int>, ApplicationRoleClaim, IdentityUserToken<int>>
{
    ...
    {
        var auditEntries = HandleAuditingBeforeSaveChanges(_currentUser.GetBankID());
    ...
    private List<AuditTrail> HandleAuditingBeforeSaveChanges(string bankID)
    {
        foreach (var entry in ChangeTracker.Entries<IAuditableEntity>().ToList())
        {
            switch (entry.State)
            {
                case EntityState.Added:
                    entry.Entity.CreatedBy = bankID;
                    entry.Entity.LastModifiedBy = bankID;
                    break;
            ...
            .ToList())
        {
            ...
            foreach (var property in entry.Properties)
            {
                ...
                {
                    trailEntry.KeyValues = property.CurrentValue?.ToString();
                    continue;
                }
    ...
    private Task HandleAuditingAfterSaveChangesAsync(List<AuditTrail> trailEntries, CancellationToken cancellationToken = new())
    {
        ...
        foreach (var entry in trailEntries)
        {
            foreach (var prop in entry.TemporaryProperties)
            {
                ...
                {
                    entry.KeyValues = prop.CurrentValue?.ToString();
                }
                else
                {
